...
function AddToStudent() {
  ...
  useEffect(() => {
    ...
    fetch('https://localhost:7103/api/sanctions')
      .then((response) => response.json())
      .then((data) => {
        const filteredSanctions = data.filter((sanction) => sanction.status === 1 && sanction.type === "Achievement");
  ...
  return (
    <div className="App bg-green">
      <h2 className="mx-3 p-3 text-center">Añadir Logro a Estudiante</h2>
      <div className="App-header d-block">
        <div className="row mx-5 p-5">
          <form className="col-md-5 mx-5" onSubmit={handleSubmit}>
            ...
            <div className="form-group">
              <label htmlFor="penaltyType">Logro:</label>
              <select required
