...
function Academic() {
  ...
  useEffect(() => {
    // Realizar una solicitud GET a la API para obtener los datos de las unidades académicas
  ...
  const handleDelete = (academicUnityId) => {
    ...
    if (confirmDelete) {
      // Realizar una solicitud DELETE para eliminar la unidad académica con el ID proporcionado
      fetch(`https://localhost:7103/api/AcademicUnities/${academicUnityId}`, {
        method: 'DELETE',
      })
        .then((response) => {
          if (response.ok) {
            // Si la eliminación es exitosa, actualiza la lista de unidades académicas
